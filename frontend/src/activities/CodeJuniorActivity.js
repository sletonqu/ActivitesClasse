...
const CodeJuniorActivity = ({ student, content = {}, onComplete }) => {
  ...
  return (
    <div id="code-junior-activity-wrapper" className="flex flex-col h-full bg-slate-50 text-slate-800">
      {/* Header */}
      <div id="code-junior-header" className="mb-4">
        <h2 id="code-junior-title" className="text-2xl font-bold text-slate-900">{title}</h2>
        <p id="code-junior-description" className="text-lg text-slate-600 mt-1">{description}</p>
      </div>

      {/* Main Content Area */}
      <div id="code-junior-content-area" className="flex-1 w-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative" style={{ minHeight: "600px" }}>
        <iframe
      ...
      {!completed && (
        <div id="code-junior-validation-wrapper" className="mt-6 flex justify-center pb-4">
          <button
            id="code-junior-validation-btn"
            onClick={handleComplete}
            className="group relative inline-flex items-center justify-center px-8 py-4 font-bold text-white transition-all duration-200 bg-green-500 font-pj rounded-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-600 hover:bg-green-600 active:scale-95 shadow-md hover:shadow-lg"
          >
