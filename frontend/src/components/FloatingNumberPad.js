...
const FloatingNumberPad = ({
  ...
  return (
    <div
      ...
      className="pointer-events-none fixed inset-0 z-50 flex items-end justify-center p-2 sm:justify-end"
    >
      <section
        ...
        className={`pointer-events-auto w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-4 shadow-2xl ${
      ...
      >
        <div
          ...
          className="mb-3 flex cursor-grab items-center justify-center rounded-full border border-stone-700/80 px-3 py-2 shadow-inner touch-none active:cursor-grabbing"
          style={{
        ...
        >
          ...
          <span aria-hidden="true" className="h-1.5 w-16 rounded-full bg-white/20 shadow-inner" />
        </div>

        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wide text-indigo-600">Pavé numérique</p>
            <p id="floating-number-pad-active-field" className="text-sm text-slate-600">
              {activeFieldLabel || "Sélectionne une case à compléter."}
            </p>
          </div>
        </div>

        <div id="floating-number-pad-grid" className="mt-4 grid grid-cols-3 gap-2">
            ...
            .map((keyValue) => {
              return (
                <button
                  ...
                  className={`inline-flex min-h-12 items-center justify-center rounded-xl border border-indigo-200 bg-indigo-50 text-xl font-bold text-slate-800 shadow-sm transition hover:bg-indigo-100 ${keyValue === "=" ? "col-span-3" : ""}`}
                >
                  {keyValue}
                </button>
              );
        ...
        <div id="floating-number-pad-actions" className="mt-3 grid grid-cols-2 gap-2">
          <button
            id="floating-number-pad-backspace-button"
            type="button"
            onClick={onBackspace}
            className="inline-flex items-center justify-center rounded-xl bg-amber-100 px-3 py-2 font-semibold text-amber-900 shadow-sm transition hover:bg-amber-200"
          >
            ⌫ Retour
          </button>
          <button
            ...
            type="button"
            onClick={onClose}
            className="inline-flex items-center justify-center rounded-xl bg-slate-700 px-3 py-2 font-semibold text-white shadow-sm transition hover:bg-slate-800"
          >
            Fermer
          </button>
        </div>
