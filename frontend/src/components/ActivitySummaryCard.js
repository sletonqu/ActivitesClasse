...
const ActivitySummaryCard = ({
  ...
  return (
    <section id={id} className={`rounded-2xl border p-5 shadow-sm ${palette.wrapper}`}>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        {score !== null && score !== undefined ? (
          <div className={`rounded-2xl border px-4 py-3 text-center shadow-sm ${palette.card}`}>
            <p className={`text-xs uppercase tracking-wide ${palette.label}`}>{scoreLabel}</p>
            <p className="text-3xl font-bold">{score} / {scoreMax}</p>
          </div>
        ) : null}
      </div>

      {footer ? (
        <div className={`mt-4 rounded-xl border p-3 text-sm shadow-sm ${palette.card}`}>{footer}</div>
      ) : null}

      {stats.length > 0 ? (
        <div className={`mt-4 grid gap-3 ${stats.length >= 3 ? "sm:grid-cols-3" : stats.length === 2 ? "sm:grid-cols-2" : "sm:grid-cols-1"}`}>
